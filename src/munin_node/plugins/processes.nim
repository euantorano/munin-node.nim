...
when defined(windows):
  var
    TH32CS_SNAPPROCESS {.importc: "TH32CS_SNAPPROCESS", header: "TlHelp32.h".}: DWORD = 0x00000002
...
when defined(windows):
  ...
  proc processesValues(plugin: Plugin): string =
    ## Get the plugin values.
    result = "processes.value 0\L" &
      "threads.value 0\L.\L"
else:
  {.error: "Processes plugin is not implemented for your OS".}
